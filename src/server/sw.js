...
self.addEventListener('fetch', event => {
    ...
    // optimize image of webp
    if (request.headers.get('accept').includes('webp') && /\.jpe?g$|.png$/i.test(request.url)) {
        var url = request.url.replace(/\.[^\.]*?$/, '.webp')
        event.respondWith(fetch(new Request(url, Object.assign({}, request))));
    }
